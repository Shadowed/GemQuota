...
function GemCount:Initialize()
	...
	self.db = self:InitializeDB( "GemCountDB", self.defaults )

	self.cmd = self:InitializeSlashCommand( L["Gem Counter commands"], "GemCount", "gemcount" );
	self.cmd:RegisterSlashHandler( "on - Enables Gem Counter", "on", function()
		if( not GemCount.db.profile.enabled ) then
			GemCount.db.profile.enabled = true;

			GemCount:Enable();
			GemCount:Print( L["Is now enabled!"] );
		end
	end );

	self.cmd:RegisterSlashHandler( "off - Disables Gem Counter", "off", function()
		if( GemCount.db.profile.enabled ) then
			GemCount.db.profile.enabled = false;

			GemCount:Disable();
			GemCount:Print( L["Is now disabled!"] );
		end
	end );
	
	self.cmd:InjectDBCommands( self.db, "copy", "list", "set" );
	...
	PaperDollFrame:HookScript( "OnShow", function()
		...
		GemCount:ScanAndUpdate();

		GemCount:RegisterEvent( "PLAYER_DAMAGE_DONE_MODS", "ScanAndUpdate" );
		GemCount:RegisterEvent( "UNIT_STATS", "ScanAndUpdate" );
	end );
	
	PaperDollFrame:HookScript( "OnHide", function()
		GemCount:UnregisterAllEvents();
	end );
end
...
function GemCount:Disable()
	self:UnregisterAllEvents();
	
	for id, option in pairs( PLAYERSTAT_DROPDOWN_OPTIONS ) do
		if( option == "PLAYERSTAT_GEM_COUNT" ) then
			table.remove( PLAYERSTAT_DROPDOWN_OPTIONS, id );
			return;
		end
	end
end
...
function GemCount:ScanAndUpdate()
	GemCount:ScanItems();
